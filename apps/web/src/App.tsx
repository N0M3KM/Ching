...
function LearnScreen({
  ...
  const [lessonIds, setLessonIds] = useState<Partial<Record<string,string>>>({});
  const selectedLesson = activeTrack?.lessons.find(lesson => lesson.id === lessonIds[activeTrack.id]) ?? activeTrack?.lessons[0];
  return (
    <>
        ...
        {activeTrack ? (
          <div className="course-card">
            <div className="course-intro">
              ...
              <label htmlFor="hsk-lesson">{locale === "en" ? "Lesson" : locale === "zh-Hant" ? "課程" : "课程"}</label>
              <select id="hsk-lesson" value={selectedLesson?.id ?? ""} onChange={event => setLessonIds(previous => ({...previous,[activeTrack.id]:event.target.value}))}>
                {activeTrack.lessons.map(lesson => <option key={lesson.id} value={lesson.id}>{lesson.title[locale]}</option>)}
              </select>
              <h3>{selectedLesson?.title[locale]}</h3>
            ...
            {(selectedLesson ? [selectedLesson] : []).map((lesson) => (
  ...
  return (
    <footer>
      ...
      <details>
        ...
        <p>Primary Mandarin speech: Qwen3-TTS (Apache-2.0), Serena. Local fallback: eSpeak NG / Echogarden (GPL-3.0).</p>
        <p>HSK vocabulary: <a href="https://github.com/drkameleon/complete-hsk-vocabulary">Complete HSK Vocabulary</a>, Yanis Zafirópulos and contributors (MIT); CC-CEDICT-derived definitions (CC BY-SA 4.0). Practice prompts generated by Ching.</p>
